...
import { importPdfOrder, PdfImportError, type PdfImportInput } from "../../../lib/supabase/pdf-orders-repository";
...
export function ImportPdfPage() {
  ...
  const [preview, setPreview] = useState<PdfOrderPreview | null>(null);
  const previewRef = useRef<PdfOrderPreview | null>(null);
  const reading = useRef<AbortController | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const uploadLock = useRef(false);
  const frozenInput = useRef<PdfImportInput | null>(null);
  ...
  async function chooseFile(file?: File) {
    ...
    setLoading(true); setError(""); setReviewed(false); setProgress("Leyendo el PDF en este dispositivo…");
    try {
      const { readPdfOrder } = await import("../../../lib/pdf-order-reader");
      const result = await readPdfOrder(file, controller.signal, setProgress);
      ...
      const existing = orders.find((order) => order.sourceSystem === "PDF" && order.sourceOrderId === result.hash);
      ...
      if (!controller.signal.aborted) {
        const message = cause instanceof Error ? cause.message : "No se pudo leer el PDF. Probá con otro archivo.";
        setError(message); sileo.error({ title: "No se pudo reconocer el PDF", description: message });
      }
  ...
  const existing = preview && orders.find((order) => order.sourceSystem === "PDF" && order.sourceOrderId === preview.hash && order.sourceStatus === "pdf_complete");
  ...
  const valid = preview && clientName.trim() && clientName.trim().length <= 80 && targetName && targetName.length <= 80 && !invalidProducts && units <= 200 && reviewed && !existing;
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!frozenInput.current) {
      frozenInput.current = {
        hash: preview.hash, file: preview.file, clientName: clientName.trim(), folderId: folderId || null, folderName: targetName,
        ...
        uploads: preview.products.flatMap((product, index) => Array.from({ length: product.quantity }, (_, unit) => ({
          file: new File([product.file], `pdf-${String(index + 1).padStart(3, "0")}-${product.code.replace(/[^a-zA-Z0-9-]/g, "-")}-${unit + 1}.jpg`, { type: "image/jpeg" }),
    ...
    try {
      const result = await importPdfOrder(frozenInput.current, setProgress, setReservedId);
      await refreshOrders();
      sileo.success({ title: result.reused ? "Este PDF ya estaba importado" : "Pedido importado", description: result.reused ? "Abrimos el pedido existente, sin crear un duplicado." : `${units} cuadros guardados en ${targetName}.` });
      router.replace(`/pedidos/${encodeURIComponent(result.orderId)}`);
    } catch (cause) {
      if (cause instanceof PdfImportError && cause.orderId) setReservedId(cause.orderId);
  ...
  return (
    <section className="mx-auto w-full max-w-[1000px]" aria-labelledby="pdf-import-title">
      ...
      <div className={`${ui.pageCard} mb-4`}>
        <div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#e8f0ea] text-[#235c4c]"><FileIcon /></span><div><p className={ui.eyebrow}>Nuevo pedido desde un documento</p><h1 id="pdf-import-title" className="text-xl font-semibold">Importar PDF</h1><p className="mt-2 text-xs leading-relaxed text-[#68726d]">Revisá los datos y elegí la carpeta. No se sube nada hasta que confirmes.</p></div></div>
        <input ref={inputRef} type="file" accept="application/pdf,.pdf" className="sr-only" aria-label="Seleccionar PDF de pedido" disabled={saving || Boolean(reservedId)} onChange={(event) => { void chooseFile(event.target.files?.[0]); event.target.value = ""; }} />
        {!preview && !loading && <button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (event.dataTransfer.files.length === 1) void chooseFile(event.dataTransfer.files[0]); else setError("Elegí un solo PDF por pedido."); }} className={`${ui.uploadZone} mt-5 w-full cursor-pointer transition-colors hover:bg-[#eef4ee] focus-visible:outline-2 focus-visible:outline-[#235c4c]`}><UploadIcon /><strong>Elegir PDF o arrastrarlo acá</strong><span>Pedidos de MAVA con texto seleccionable · Hasta 20 MB / 20 páginas</span></button>}
        ...
        {preview && <div className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-[#f3f6f2] px-3 py-2"><span className="min-w-0 break-all text-xs text-[#68726d]">{preview.file.name} · {preview.pages.length} páginas</span><button type="button" className={ui.textButton} disabled={locked} onClick={() => inputRef.current?.click()}>Cambiar PDF</button></div>}
      </div>
      ...
      {preview && <form onSubmit={submit} className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)]">
        ...
        <aside className="min-w-0 space-y-4 lg:sticky lg:top-[84px] lg:self-start">
          <div className={ui.pageCard}>
            ...
            <button type="submit" disabled={!valid || saving} className={`${ui.primaryButton} mt-4 w-full`}>{saving ? <SpinnerIcon className="animate-spin" /> : <CheckIcon />}{saving ? "Importando…" : reservedId ? "Reintentar subida" : "Crear pedido desde PDF"}</button>
            <p role="status" aria-live="polite" className="mt-3 text-center text-[11px] leading-relaxed text-[#68726d]">{saving ? progress : reservedId ? "Se continuará el mismo pedido, sin duplicarlo." : "El PDF original y las fotos se guardan al confirmar."}</p>
          </div>
          <button type="button" className={`${ui.secondaryButton} w-full`} aria-expanded={showPdf} onClick={() => setShowPdf((value) => !value)}><FileIcon />{showPdf ? "Ocultar documento" : "Ver vista previa del PDF"}</button>
          {showPdf && <PdfPagePreview pages={preview.pages} />}
        </aside>
      </form>}
    </section>
  );
